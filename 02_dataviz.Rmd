...
path_green <- tech_dev %>% 
  mutate(green_path = case_when( 
    Y_tag == TRUE & period == '2' & rta_bin == 1 & rta_bin_delta == 0 & n_tech_region_delta > 0 ~ 'extension',
...
path_green %>%
  ...
  # coord_polar() +
  facet_wrap(vars(nuts), ncol = n_regions, scales = 'free_y') +
  theme(legend.position = 'bottom') +
...
Check Norway! and NUTS versions
