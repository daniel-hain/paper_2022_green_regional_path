...
tbl_region_person <- tbl_person %>%
  semi_join(tbl_select_region, by = 'nuts') %>%
  compute()
```
...
tbl_region_pers_appln <- tbl_pers_appln %>%
  inner_join(tbl_region_person %>% select(person_id, nuts), by = 'person_id') %>%
  compute()
```
...
tbl_region_appln <- tbl_appln %>%
  semi_join(tbl_region_pers_appln %>% filter(invt_seq_nr > 0), by = 'appln_id') %>%
  compute()
```
