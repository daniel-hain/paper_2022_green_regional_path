...
data_appln %<>% semi_join(data_pers_appln, by = 'appln_id')
...
```{r}
data_inv_appln <- data_pers_appln %>%
  group_by(appln_id) %>%
  mutate(n_inv = n()) %>%
  ungroup()
```
...
region_tech <- data_appln %>%
  ...
  left_join(data_inv_appln %>% select(appln_id, person_id, nuts, n_inv), by = 'appln_id') %>%
...
data_pers_appln  %>% write_rds('../temp/tbl_region_inv_appln.rds')
data_person %>% write_rds('../temp/data_inv.rds')
